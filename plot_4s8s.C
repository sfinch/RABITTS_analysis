...
void plot_4s8s(int run_num, int run_num2 = 0){
    ...
    for (int i=0; i<num_win; i++){ //window
        for (int j=0; j<2; j++){ //clover
            hEnWin[i][j] = new TH1F(Form("239Pu_Time%i_Det%i", i+1, j+1), Form("hEnWin%iDet%i", i, j), 30000, 0, 3000);
        }
        hEnWin[i][2] = new TH1F(Form("239Pu_Time%i_BothDets", i+1), Form("hEnWin%i", i), 30000, 0, 3000);
    }
    for (int i=0; i<2; i++){
